...
import { StudentNavPage } from '../types.ts';
import {
  LayoutDashboard,
  PlusCircle,
  ListOrdered,
...
export const StudentLayout: React.FC<StudentLayoutProps> = ({
  ...
  const navItems = [
    {
      id: 'dashboard' as StudentNavPage,
      label: 'Dashboard',
      icon: LayoutDashboard,
    },
    {
      id: 'raise-complaint' as StudentNavPage,
      label: 'Raise Complaint',
      icon: PlusCircle,
    },
    {
      id: 'my-complaints' as StudentNavPage,
      label: 'My Complaints',
      icon: ListOrdered,
    },
    {
      id: 'profile' as StudentNavPage,
      label: 'Profile',
      icon: UserIcon,
    },
  ];

  const handleNavClick = (page: StudentNavPage) => {
    onNavigate(page);
      ...
      <header className="md:hidden bg-[#1E3A8A] text-white px-5 py-3.5 flex items-center justify-between shadow-md z-30 sticky top-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-blue-200 uppercase tracking-widest">Student Portal</p>
          </div>
        </div>
        <button
      ...
      >
        ...
        <div className="p-6 border-b border-blue-800/50 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-blue-200 mt-1 uppercase tracking-widest">Student Portal</p>
          </div>
        </div>

        {/* Student Mini Card */}
        <div className="p-4 mx-4 mt-4 rounded-xl bg-blue-900/60 border border-blue-700/50">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[11px] text-blue-200 font-mono truncate">
                {user?.rollNumber || 'Roll No. Pending'}
              </p>
            </div>
          </div>
          {user?.department && (
            <div className="mt-2.5 pt-2 border-t border-blue-800/60 flex items-center gap-1.5 text-[11px] text-blue-200">
